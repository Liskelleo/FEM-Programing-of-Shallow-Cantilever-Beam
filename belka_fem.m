% beam.m
% 
% additional files of necessity as follow:
% ---square_node_array.m
% ---make_elem.m
% ---plot_mesh.m
% ---plot_field.m
...
numy = 4; % the number of elements in the y-direction (beam length)
numx = 18; % and in the x-direciton.
plotMesh = 1; % A flag that if set to 1 plots the initial mesh (to make sure
% that the mesh is correct)
% TIP LOAD
P = 4.5; % the magnitude of the traction (concentrated force) at the right edge
p = 1; % the peak magnitude of the triangular traction at the bottom edge
s = -5; % the magnitude of the shearing force at the top/bottom edge
G = 0; % the magnitude of the weight of beam
% STRESS ASSUMPTION
...
% no need for that, cause it can be expressed only by the parameter 'c'.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% leftEdge, topEdge, bottomEdge - I'll give you three guesses! 
% ie The answer is fairly obvious and you should guess it easily.
%
...
switch elemType
case 'Q9'
% please pay attention that the edge boundaries are degraded into 'L3' element.
% if you are still confused, please refer to 'lagrange_basis.m' for answer.
...
% or leftEdge  =[ 1:2*nnx:(uln-nnx);  1+2*nnx:2*nnx:uln; (lrn+1):2*nnx:uln ]';
% only need to pay attention to the order that corresponds to the coordinates,
% no need to pay attention to the arrangement order (eg from top to bottom),
% cause the way that the element force vector scattered into a global force 
% vector only depends on the golbal node number!
topEdge   =[ urn:-2:(uln+1); (urn-2):-2:uln; (urn-1):-2:uln ]';
bottomEdge=[ 1:2:(lrn-1); 3:2:lrn; 2:2:lrn]';
edgeElemType='L3';
...
% or leftEdge  =[ 1:nnx:(uln-nnx); lrn+1:nnx:uln ]';
topEdge   =[ urn:-1:(uln+1); (urn-1):-1:uln ]';
bottomEdge=[ 1:1:(lrn-1); 2:1:lrn]';
edgeElemType='L2';
end
...
%fixedNodeX=[ 1:nnx:uln ]'; % if gravity is applied, switch code to this line
% the x direction
...
sctry=sctrx+numnode; % y scatter vector, dof mapping!
...
fxPt=-(3*yPt*p*(c^2 - (5*yPt^2)/3))/(10*c^3);
fyPt=(-c^4*p + 40*s*yPt*c^2*L + 6*p*yPt^2*c^2 - 5*p*yPt^4 + 30*P*L*yPt^2 - 30*c^2*P*L)/(40*c^3*L);
...
sctr=leftEdge(e,:); % why rightEdge??? must be leftEdge!
sctrx=sctr;
sctry=sctrx+numnode;
for q=1:size(W,1) % quadrature loop
pt=Q(q,:); % quadrature point
...
fxPt=-(-10*L*(-(3*p*(c^2 - (5*yPt^2)/3)*yPt)/(10*c^3*L))*c^3 + 10*s*L*c^2 - 3*p*yPt*c^2 - 5*p*L^2*yPt + 5*p*yPt^3 + 15*P*L*yPt)/(10*c^3);
fyPt=-(-(3*c^2*P*L)/4 - ((c^2 - 15*L^2 - 5*yPt^2)*p*(c^2 - yPt^2))/40 + ((4*c^2*s + 3*P*yPt)*L*yPt)/4)/(c^3*L);
...
for e=1:size(topEdge,1) % loop over the elements in the top edge
    sctr=topEdge(e,:);
    sctrx=sctr;
    sctry=sctrx+numnode;
    for q=1:size(W,1) % quadrature loop
    ...
    detJ0=norm(J0); % determiniat of jacobian
    xPt=N'*node(sctr,1);
    fxPt=s; % x traction at quadrature point
    f(sctrx)=f(sctrx)+N*fxPt*detJ0*wt;
    end % of quadrature loop
    end % of element loop
...
for e=1:size(bottomEdge,1) % loop over the elements in the top edge
    sctr=bottomEdge(e,:);
    sctrx=sctr;
    sctry=sctrx+numnode;
    for q=1:size(W,1) % quadrature loop
    ...
    detJ0=norm(J0); % determiniat of jacobian
    xPt=N'*node(sctr,1);
    fyPt=xPt*(p/L); % y traction at quadrature point
    fxPt=s; % x traction at quadrature point
    f(sctry)=f(sctry)+N*fyPt*detJ0*wt;
    f(sctrx)=f(sctrx)+N*fxPt*detJ0*wt;
    end % of quadrature loop
...
% GRAVITY INFLUENCE ---optional
for e=1:size(element,1) % loop over the elements in the top edge
    sctr=element(e,:);
    sctrx=sctr;
    sctry=sctrx+numnode;
    for q=1:size(W,1) % quadrature loop
    pt=[Q(q,:),Q(q,:)]; % quadrature point
    wt=W(q); % quadrature weight
    [N,dNdxi]=lagrange_basis(elemType,pt); % element shape functions
    J0=dNdxi'*node(sctr,:); % element Jacobian
    detJ0=norm(J0); % determiniat of jacobian
    fyPt=-G; % y traction at quadrature point
    f(sctry)=f(sctry)+N*fyPt*detJ0*wt;
    end % of quadrature loop
    end % of element loop
